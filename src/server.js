...
function cardSvg(res,id){
  ...
  res.end(`<svg xmlns="http://www.w3.org/2000/svg" width="960" height="540" viewBox="0 0 960 540">
    <defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#101722"/><stop offset="1" stop-color="#05070b"/></linearGradient></defs>
...
async function cardWebp(res,id){
  const c=channels.find(x=>x.id===id);
  if(!c){res.writeHead(404);return res.end();}
  try{
    const buf=await renderChannelCard(c);
    res.writeHead(200,{
    ...
    console.log(`CARD_RENDER_ERROR ${id} ${e.name}: ${e.message}`);
    return json(res,500,{error:'card render failed'},0);
  }
}
...
async function runTempCandidateProbe(){
  const urls=[
    "https://stream1.freetv.fun/94fb737967922381fad072d73b06515fbd2bb7c2d46e8458f16dbc0cfbd3f87c.m3u8",
    "https://stream1.freetv.fun/582197447e32b2b186bb7b123625e1a24f66567003a8d99154fdad7a5808dff1.m3u8",
    "https://stream1.freetv.fun/239865eb44f2ac54627025e83a852da8a3296a2aaa7820876e726e1e9ac6e901.m3u8",
    "https://stream1.freetv.fun/3c9ed2cfd07b47b504e4c4e2083c7c0f01bb9bd8a645bb2c15f49fee84c1729c.m3u8",
    "https://stream1.freetv.fun/3e0d3c58c379a71f9385c73ae2b2fdfd77bb8def82b80728d980632cfc82d15c.m3u8",
    "https://stream1.freetv.fun/3a88f55139022e90fe1d9729ad32a8579543da9dcd490926ab9f6e2b40049a0a.m3u8",
    "https://stream1.freetv.fun/559041fa1cd5a148d80750f412178611fbe15f77b72accf7fbba42cf0c1ae210.m3u8",
    "https://stream1.freetv.fun/3d1106701b5447b6c0813bd535bcd2c708950cc54ddb9b7d6872fc48f9b45c7c.m3u8",
    "https://stream1.freetv.fun/b08a9fd2ebd28f0e907ebfd39363bcf59efae565c1328baa25d8706e6912e972.m3u8"
  ];
  const results=await Promise.all(urls.map(async(url,index)=>{const ctl=new AbortController();const t=setTimeout(()=>ctl.abort(),10000);try{const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0','accept':'application/vnd.apple.mpegurl,*/*'},signal:ctl.signal});const body=await r.text();return{index:index+1,status:r.status,ok:r.ok,hls:/#EXTM3U/.test(body),contentType:r.headers.get('content-type'),finalHost:new URL(r.url).host};}catch(e){return{index:index+1,status:0,ok:false,hls:false,error:e.name+': '+e.message};}finally{clearTimeout(t);}}));
  console.log('TEMP_CANDIDATE_PROBE '+JSON.stringify(results));
}
...
server.listen(PORT,'0.0.0.0',()=>{
  ...
  runTempCandidateProbe().catch(e=>console.log('TEMP_CANDIDATE_PROBE_ERROR '+e.message));
});
