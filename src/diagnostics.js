...
async function inspectTv3Live(){const page='https://tv3live.my/tv3/';const out={page};try{const a=await getText(page);out.status=a.status;out.ms=a.ms;out.finalUrl=a.url;const urls=extractUrls(a.text,a.url);out.mediaCandidates=urls.filter(u=>/\.m3u8(?:[?#]|$)|\.mpd(?:[?#]|$)|dailymotion|akamaized|cloudfront|player|video|stream/i.test(u)).slice(0,40);out.iframes=[...a.text.matchAll(/<iframe[^>]+src=["']([^"']+)["']/gi)].map(m=>resolve(a.url,m[1])).filter(Boolean).filter(safeCandidate).slice(0,20);const scripts=urls.filter(u=>/\.js(?:[?#]|$)/i.test(u)).slice(0,8);out.scriptCandidates=[];for(const s of scripts){try{const js=await getText(s,a.url);for(const u of extractUrls(js.text,js.url)){if(/\.m3u8(?:[?#]|$)|\.mpd(?:[?#]|$)|dailymotion|akamaized|cloudfront|player|video|stream/i.test(u)&&safeCandidate(u))out.scriptCandidates.push(u)}}catch{}}out.scriptCandidates=[...new Set(out.scriptCandidates)].slice(0,40);return out}catch(e){out.error=e.name+': '+e.message;return out}}
